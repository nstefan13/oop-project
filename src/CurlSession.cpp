...
std::expected<HTTPResponse, CURLcode>
CurlSession::request(const std::string &method, const std::string &url,
                     const HTTPHeaders &_headers, const std::string &body) {
  ...
  auto read_callback = [](char *buffer, size_t size, size_t nitems,
                          void *user_data_ptr) -> size_t {
    _user_data_read *user_data = static_cast<_user_data_read *>(user_data_ptr);
    size_t ncopied =
        size * std::min<size_t>(
                   nitems, user_data->original_string->size() -
                               user_data->position); // altough size is always
                                                     // 1, as stated in the docs
    if (ncopied == 0)
      return 0;

    std::memcpy(buffer,
                user_data->original_string->data() + user_data->position,
                ncopied);
  ...
  auto write_callback = [](char *buffer, size_t size, size_t nitems,
                           void *user_data_ptr) -> size_t {
    _user_data_write *user_data =
        static_cast<_user_data_write *>(user_data_ptr);
    for (size_t i = 0; i < nitems; i++) {
      user_data->response->raw_body.push_back(
          *reinterpret_cast<std::byte *>(buffer + i * size));
    }
    return nitems;
  };

  auto write_headers_callback = [](const char *buffer, size_t size,
  ...
  if (!method.size())
    throw MissingNetworkParameterException(
        "CurlSession::request: method must not be empty");
  curl_easy_setopt(handle, CURLOPT_CUSTOMREQUEST, method.data());

  if (!url.size())
    throw MissingNetworkParameterException(
        "CurlSession::request: url must not be empty");
  ...
  if (body.size()) {
    ...
    curl_easy_setopt(
        ...
        +read_callback); // + for explicit decay to function pointer
...
CurlSession::CurlSession() : handle{curl_easy_init()} {
  curl_easy_setopt(handle, CURLOPT_TIMEOUT,
                   20L); // Set a timeout of 20 seconds to prevent hanging
...
CurlSession &CurlSession::operator=(
    CurlSession &&other) { // this must free itself and other because it
                           // basically means "overwrite this"
  if (this == &other)
    return *this;
...
CurlSession::~CurlSession() { curl_easy_cleanup(handle); }

std::ostream &operator<<(std::ostream &os, const CurlSession &self) {
  return os << std::format("CurlSession {{ latest_url={} latest_status_code={} "
                           "latest_wait_time={} }}",
                           self.getinfo_wrapper_last_url(),
                           self.getinfo_wrapper_last_status_code(),
                           self.getinfo_wrapper_total_time());
}
