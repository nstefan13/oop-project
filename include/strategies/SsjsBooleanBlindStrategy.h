...
public:
  ...
  [[nodiscard]] const std::string &getDescription() const override {
    static const std::string desc = 
        "Compares the response of a known FALSE injection against a known TRUE injection. "
        "If the responses differ significantly, it confirms a boolean blind vulnerability "
      ...
      for (const auto &character : Constants::Escapes) {
        for (const auto &[falseLogic, trueLogic] : Constants::BooleanBlinds) {
          ...
          for (const auto &suffix : Constants::Suffixes) {
            std::string falsePayload = concat(value, character, falseLogic, suffix);
            std::string truePayload = concat(value, character, trueLogic, suffix);
            payloadPairs.emplace_back(falsePayload, truePayload);
          }
          
          // Balance the syntax by ORing the rest
          std::string falsePayloadOR = concat(value, character, falseLogic, " || ", character, value);
          std::string truePayloadOR = concat(value, character, trueLogic, " || ", character, value);
          payloadPairs.emplace_back(falsePayloadOR, truePayloadOR);

          for (const auto &[falsePayload, truePayload] : payloadPairs) {
            auto falseResponse = this->requestBlueprint.compileRequest(falsePayload).perform();
            if (!falseResponse.has_value())
              continue;

            auto trueResponse = this->requestBlueprint.compileRequest(truePayload).perform();
            ...
            bool statusCodeDiffers = falseResponse->get_status_code() != trueResponse->get_status_code();
